...
import { addNewEmployee, getAllEmployee, updateEmployee } from '../../slices/employee/thunk.ts';
...
export const AddUser = ({ modalOpen, setModalOpen, item, setItem }: any) => {
  ...
  const { loading, isAction, isSuccess } = useSelector((state: any) => state.Employees);
  ...
  const newStations: any = stations?.map(({
                                            name: label,
                                            id: value, ...rest
                                          }: { name: string, id: string }) => ({ label, value, ...rest }));
  ...
  const handleImageChange = async (event: any) => {
    ...
    try {
      const responseImg: any = await axios.post(baseUrl + '/attachment/upload', formData, {
        headers: {
  ...
  const validation: any = useFormik({
    ...
    onSubmit: (values) => {
      if (item) {
        dispatch(
          updateEmployee(
            { ...values, attachmentId: image, id: item.id }
          )
        );
      } else {
        dispatch(addNewEmployee({
          ...values, attachmentId: image
        }));
      }
          ...
          >
            ...
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div className="divide-y divide-blue-200">
                <h5 className={'text-title-md font-semibold text-black dark:text-white '}>Xodim yaratish</h5>
              </div>
              <div className={'sm:flex gap-3'}>
                <div
                  ...
                  <div className="relative  drop-shadow-2">
                    <img className={'rounded-full object-cover'} width={320} height={320}
                         src={image ? `${baseUrl}/attachment/download/${image}` : userSix}
                         alt="profile" />
                    <label
                      htmlFor="profile"
              ...
              >
                ...
                <div className={'sm:flex  gap-3'}>
                  <div className={'my-2 sm:w-1/2'}>
                    <label htmlFor="roleId" className="block text-md font-medium leading-6 text-gray-900">
                      Lavozim
                    </label>
                    <div className="mt-2">
                ...
                <div className={'sm:flex  gap-3'}>
                  <div className={'my-2 w-full'}>
                    <label htmlFor="stationIdList" className="block text-md font-medium leading-6 text-gray-900">
                      Stansiya
                    </label>
                    <div className="mt-2">
                      <div className="relative inline-block w-full">
                        <Select
                          ...
                          value={newStations?.filter((option: any) =>
                            validation.values.stationIdList.includes(option.value)
                          )}
                          isMulti={true}
                    ...
                    {validation.touched.stationIdList && validation.errors.stationIdList ? (
                      <h6 className="block text-md font-medium leading-6 text-red-900">
                ...
                <div className={'sm:flex  gap-3'}>
                  <div className={'my-2 sm:w-1/2'}>
                    <label htmlFor="jshshir" className="block text-md font-medium leading-6 text-gray-900">
                      JSHSHIR
                    </label>
                    <div className="mt-2">
                      <input
