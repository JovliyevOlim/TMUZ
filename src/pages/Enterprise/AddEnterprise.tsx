...
import { addNewEnterprise, updateEnterprise } from '../../slices/enterprise/thunk.ts';
import { Dialog, DialogBackdrop, DialogPanel } from '@headlessui/react';


export const AddEnterprise = ({ modalOpen, setModalOpen, item, setItem }: any) => {
  const dispatch: any = useDispatch();
  const { loading, isAction, isSuccess } = useSelector((state: any) => state.EnterPrise);
  const { mtu } = useSelector((state: any) => state.Mtu);
  ...
  const validation: any = useFormik({
    ...
    onSubmit: (values) => {
      if (item) {
        dispatch(updateEnterprise({ ...values, id: item.id }));
      } else {
        dispatch(addNewEnterprise(values));
      }
    }
  });
            ...
            >
              <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div className="divide-y divide-blue-200">
                  <h5 className={'text-title-md font-semibold text-black dark:text-white '}>Korxona yaratish</h5>
                </div>
                <div className={'my-2'}>
                ...
                <div className={'my-2'}>
                  <label htmlFor="mtuId" className="block text-md font-medium leading-6 text-gray-900">
                    Mtu
                  </label>
                  <div className="mt-2">
                  ...
                  {validation.touched.mtuId && validation.errors.mtuId ? (
                    <h6 className="block text-md font-medium leading-6 text-red-900">
                      {validation.errors.mtuId}
                    </h6>
                  ) : null}
                </div>
