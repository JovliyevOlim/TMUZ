...
import { addNewPeregon, updatePeregon } from '../../slices/peregon/thunk.ts';


export const AddPeregon = ({ modalOpen, setModalOpen, item, setItem }: any) => {
  const dispatch: any = useDispatch();
  const { loading, isAction, isSuccess } = useSelector((state: any) => state.Peregon);
  ...
  const validation: any = useFormik({
    ...
    validationSchema: Yup.object({
      name: Yup.string().required('Peregon nomini kiriting!'),
      plotId: Yup.string().required('Uchastkani  tanlang!')
    }),
    onSubmit: (values) => {
      if (item) {
        dispatch(updatePeregon({ ...values, id: item.id }));
      } else {
        dispatch(addNewPeregon(values));
      }
    }
  });
            ...
            >
              <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                <div className="divide-y divide-blue-200">
                  <h5 className={'text-title-md font-semibold text-black dark:text-white '}>Peregon yaratish</h5>
                </div>
                <div className={'my-2'}>
